...
export async function POST(request) {
  try {
    ...
    // 3. SMART ROUTING: Find which shop this order belongs to via Bot Token in URL
    ...
    // 5. DATABASE INSERT WITH ROUTING
    let orderId = null;
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          address: address,
          status: 'Pending',
          shop_owner_email: targetShopEmail // ✅ MAGIC: Order sahi shop ke paas jayega
        })
        .select();
    ...
    } catch (dbErr) { console.error('❌ DB Crash:', dbErr); }
